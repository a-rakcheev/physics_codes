...
for k, q_set in enumerate(ql):
    ...
    data = np.load(prefix + "nonreciprocal_dynamics_pair_q=" + str(q_set).replace(".", "-") + "/nonreciprocal_dynamics_with_sia_pair_energy_q=" + str(q_set).replace(".", "-") + "_z0=" + str(z_0_set).replace(".", "-")
        + "_theta=" + str(theta) + "_eta=" + str(eta).replace(".", "-") + "_tf=" + str(t_final).replace(".", "-") + "_t_steps=" + str(t_steps) + "_f_steps=" + str(factor_steps) + ".npz")
...
# colorbar
ax = fig.add_axes([0.07, 0.08, 0.9, 0.02])
cb = fig.colorbar(plot, cax=ax, orientation="horizontal")

cb.ax.tick_params(labelsize=ticksize)
plt.subplots_adjust(wspace=0.25, hspace=0.2, left=0.07, right=0.97, bottom=0.3, top=0.925)
plt.savefig("Fig10.png", format="png", dpi=1200)
plt.show()