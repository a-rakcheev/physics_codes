...
for k in np.arange(2, l + 1, 1):
    ...
    op_file = prefix + "operators/operators_TPFY_l" + str(k) + ".txt"
...
plt.grid()
plt.quiver(X, Y, minor_u, minor_v, norm, norm=colors.LogNorm(vmin=10 ** -3, vmax=10 ** -1), cmap=cmap, pivot="mid")
# plt.quiver(X, Y, minor_u, minor_v, norm, norm=colors.LogNorm(vmin=10 ** -3, vmax=10 ** 0), cmap=cmap, pivot="mid")
