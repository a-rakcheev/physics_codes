...
def create_P_matrix_symmetric_TP_multiprocessing(index):

    i = index
    print("i", i, flush=True)

    for j in range(i, size):
        comm_label_i = C_zz_labels[i, :, :]
        comm_coeff_i = C_zz_coeffs[i, :]
...
if __name__ == "__main__":
    ...
    P = mp.Array('d', (int(size) * (int(size) + 1)) // 2)
    pool = mp.Pool(processes=number_of_processes)
    comp = pool.map_async(create_P_matrix_symmetric_TP_multiprocessing, [i for i in range(size)])
    comp.wait()
    pool.terminate()
