...
with zipfile.ZipFile(name_zip) as zipper:
    ...
    # R matrices
    name = "optimization_matrices_R_" + op_name_1.upper() + "_" + op_name_0.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    ...
    name = "optimization_matrices_R_" + op_name_1.upper() + "_" + op_name_1.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        data = np.load(f)
        R_1_1 = data["R"] * factor_1 * factor_1

    name = "optimization_matrices_R_" + op_name_1.upper() + "_" + op_name_2.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        data = np.load(f)
        R_1_2 = data["R"] * factor_1 * factor_2

    name = "optimization_matrices_R_" + op_name_2.upper() + "_" + op_name_0.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        data = np.load(f)
        R_2_0 = data["R"] * factor_2 * factor_0

    name = "optimization_matrices_R_" + op_name_2.upper() + "_" + op_name_1.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        data = np.load(f)
        R_2_1 = data["R"] * factor_2 * factor_1

    name = "optimization_matrices_R_" + op_name_2.upper() + "_" + op_name_2.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        data = np.load(f)
    ...
    # P matrices
    # symmetric
    name = "optimization_matrices_P_" + op_name_0.upper() + "_" + op_name_0.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        P_0_0 = sp.load_npz(f) * factor_0 * factor_0

    name = "optimization_matrices_P_" + op_name_1.upper() + "_" + op_name_1.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        P_1_1 = sp.load_npz(f) * factor_1 * factor_1

    name = "optimization_matrices_P_" + op_name_2.upper() + "_" + op_name_2.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        P_2_2 = sp.load_npz(f) * factor_2 * factor_2

    # not symmetric, need to check if operators are ordered correctly
    name = "optimization_matrices_P_" + op_name_0.upper() + "_" + op_name_1.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    if name not in zipper.namelist():
        name = "optimization_matrices_P_" + op_name_1.upper() + "_" + op_name_0.upper() + "_TPFXY_l=" + str(l) + ".npz"

    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        P_0_1 = sp.load_npz(f) * factor_0 * factor_1
    P_0_1 = P_0_1 + P_0_1.T

    name = "optimization_matrices_P_" + op_name_0.upper() + "_" + op_name_2.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    if name not in zipper.namelist():
        name = "optimization_matrices_P_" + op_name_2.upper() + "_" + op_name_0.upper() + "_TPFXY_l=" + str(l) + ".npz"

    with io.BufferedReader(zipper.open(name, mode='r')) as f:
        P_0_2 = sp.load_npz(f) * factor_0 * factor_2
    P_0_2 = P_0_2 + P_0_2.T

    name = "optimization_matrices_P_" + op_name_1.upper() + "_" + op_name_2.upper() + "_TPFXY_l=" + str(
        l) + ".npz"
    if name not in zipper.namelist():
        name = "optimization_matrices_P_" + op_name_2.upper() + "_" + op_name_1.upper() + "_TPFXY_l=" + str(l) + ".npz"
