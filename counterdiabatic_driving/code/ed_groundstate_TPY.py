...
res_1 = 200                                        # number of grid points on x axis
res_2 = 200                                       # number of grid points on y axis
...
# # hamiltonians (if complex)
# mat_name = prefix + op_name_0 + "_TP_L=" + str(L) + "_k=" + k_name + "_p=" + str(par) + ".npz"
# data = np.load(mat_name)
# indptr = data["indptr"]
# indices = data["idx"]
# val = data["val"]
# h_0 = factor_0 * sp.csr_matrix((val, indices, indptr), shape=(size, size))
#
# mat_name = prefix + op_name_1 + "_TP_L=" + str(L) + "_k=" + k_name + "_p=" + str(par) + ".npz"
# data = np.load(mat_name)
# indptr = data["indptr"]
# indices = data["idx"]
# val = data["val"]
# h_1 = factor_1 * sp.csr_matrix((val, indices, indptr), shape=(size, size))
#
# mat_name = prefix + op_name_2 + "_TP_L=" + str(L) + "_k=" + k_name + "_p=" + str(par) + ".npz"
# data = np.load(mat_name)
# indptr = data["indptr"]
# indices = data["idx"]
# val = data["val"]
# h_2 = factor_2 * sp.csr_matrix((val, indices, indptr), shape=(size, size))
#
# evals = np.zeros((res_1, res_2))
# evecs = np.zeros((res_1, res_2, size), dtype=np.complex128)
