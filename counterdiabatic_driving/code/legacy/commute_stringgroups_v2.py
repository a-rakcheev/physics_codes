...
try:
    ...
    Warning('[mpnum] Library not installed. Cannot make MPOs!')
    Warning('TRY: [ https://github.com/dseuss/mpnum ]')
...
class equation(dict):
    ...
    def make_operator(self,):
        '''
        Makes the actual sparse matrix associated with a string.

        !! Unknown behavior for arrays !!
        '''
        str_in = self
        if str_in=={}:
            return 0
        if len(str_in.keys()[0])>20:
            raise BaseException("Matrix is too big! ABORTING!")

        sp_out1 = 0*local_op.locop('i',0,len(str_in.keys()[0]))
        for ele in str_in:
            str_in2 = ele.replace('1','i')
            sp_out = local_op.locop('i',0,len(ele))
            for i in range(len(ele)):
                #print str_in2[i]
                if str_in2[i]!='i':
                    sp_out = sp_out.dot(local_op.locop(str_in2[i],i,len(ele)))
            sp_out1 += sp_out*str_in[ele]
        return sp_out1

    def make_MPO(self,):
        N = len(self.keys()[0])
        h_mpo = 0*mp.MPArray.from_kron([locop('i',0,1).toarray()]*(N))
        k = 0
        for term in tqdm.tqdm(self.keys()):
            h_mpo += self[term]*mp.MPArray.from_kron([locop(term[i],0,1).toarray() for i in range(N)])
            k += 1
            if k%10==0:
                er = h_mpo.compress(relerr=1e-10)
        er = h_mpo.compress(relerr=1e-10)
        return h_mpo
...
class maths:
    ...
    # def from_matrix(self,operator,max_extent=2,precision=1e-4):
    #     N = int(log2(operator.shape[0]))
    #     obs_out = []
    #     for qq in range(1,max_extent+1):
    #         for x in itertools.combinations(range(N),qq):
    #             for v in itertools.product(*[['x','y','z']]*qq):
    #                 Ss = locop('i',0,N)
    #                 for i in range(qq):
    #                     Ss = Ss.dot(locop(v[i],x[i],N))
    #
    #                 overlap = trace(Ss.dot(operator))/(2**N)
    #                 if abs(overlap)>precision:
    #                     obs_out.append([x,v,float(real(overlap))])
    #     aa_dict = {}
    #     for aa_ in obs_out:
    #         strout = list('1'*N)
    #         for qq in range(len(aa_[0])):
    #             strout[aa_[0][qq]] = aa_[1][qq]
    #         aa_dict[''.join(strout)] = aa_[2]
    #     return equation(aa_dict)
