...
xl = np.linspace(-1.0, 1.0, res_x)
yl = np.linspace(-1.0, 1.0, res_y)
...
for l, g in enumerate(xl):
    for n, h in enumerate(yl):
        ...
        tr_gg = np.dot(ground_state.T.conj(), Ag_mat.dot(ground_state))
        tr_hh = np.dot(ground_state.T.conj(), Agh_mat.dot(ground_state))
        tr_gh = np.dot(ground_state.T.conj(), Ahg_mat.dot(ground_state))
        tr_hg = np.dot(ground_state.T.conj(), Ahh_mat.dot(ground_state))
...
streamplot_metric_tensor(xl, yl, metric_grid, "inferno_r")
plt.xlabel(r"$g$", fontsize=12)
plt.ylabel(r"$h$", fontsize=12)

plt.title("Coherent Infinite Temperature Metric of the Ising Chain in Mixed Fields", fontsize=12)
plt.show()
