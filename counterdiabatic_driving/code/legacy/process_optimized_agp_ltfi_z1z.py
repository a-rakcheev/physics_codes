...
def fill_metric_chi_metric(index_tuple):
    print("i, j:", index_tuple[0], index_tuple[1])
    ...
    prod_comm_xx = comm_x * comm_x
    prod_comm_yy = comm_y * comm_y
    prod_comm_xy = comm_x * comm_y
    prod_ham_deriv_xx = ham_deriv_x * ham_deriv_x
    prod_ham_deriv_xy = ham_deriv_x * ham_deriv_y
    prod_ham_deriv_yy = ham_deriv_y * ham_deriv_y

    metric = np.zeros((2, 2))
    metric[0, 0] = prod_ham_deriv_xx.trace().real + prod_comm_xx.trace().real
    metric[1, 1] = prod_ham_deriv_yy.trace().real + prod_comm_yy.trace().real
    metric[0, 1] = prod_ham_deriv_xy.trace().real + prod_comm_xy.trace().real
...
if __name__ == '__main__':
    ...
    L = 6  # number of spins
    res_x = 100  # number of grid points on x axis
    ...
    # computation = "subspace_coherent"
    # computation = "subspace_incoherent"
    # computation = "finite_temperature_coherent"
    # computation = "infinite_temperature_coherent"
    # computation = "finite_temperature_incoherent"
    # computation = "infinite_temperature_incoherent"
    # computation = "chi_metric"
    ...
    computations = ["subspace_coherent", "finite_temperature_coherent", "infinite_temperature_coherent"]

    xl = np.linspace(1.e-6, 1.5, res_x)
    yl = np.linspace(1.e-6, 1.5, res_y)

    metric_grid = mp.Array('d', res_x * res_y * 4)
    prefix = "/home/artem/Downloads/"
    name = prefix + "optimize_agp_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) + "_res_x" + str(
        res_x) + "_res_y" + str(
        res_y) + ".pkl"
    ...
    for computation in computations[:]:
        ...
        if computation == "subspace_coherent":
            ...
            name = "metrics_subspace_coherent_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
            ...
            comp.wait()
            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_subspace_incoherent_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
                   + "_S" + str(S) + "_res_x" + str(res_x) + "_res_y" + str(res_y) + ".npz"
            np.savez_compressed(name, metric=metric)
            ...
            pool = mp.Pool(processes=number_of_processes)
            comp = pool.map_async(fill_metric_finite_temperature_coherent,
                                  [(i, j) for i in range(res_x) for j in range(res_y)])
            comp.wait()

            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_coherent_finT_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
            ...
            pool = mp.Pool(processes=number_of_processes)
            comp = pool.map_async(fill_metric_infinite_temperature_coherent,
                                  [(i, j) for i in range(res_x) for j in range(res_y)])
            comp.wait()

            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_coherent_infT_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
            ...
            pool = mp.Pool(processes=number_of_processes)
            comp = pool.map_async(fill_metric_finite_temperature_incoherent,
                                  [(i, j) for i in range(res_x) for j in range(res_y)])
            comp.wait()

            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_incoherent_finT_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
                   + "_beta" + str(beta).replace(".", "-") + "_res_x" + str(res_x) + "_res_y" + str(res_y) + ".npz"
            np.savez_compressed(name, metric=metric)
            ...
            pool = mp.Pool(processes=number_of_processes)
            comp = pool.map_async(fill_metric_infinite_temperature_incoherent,
                                  [(i, j) for i in range(res_x) for j in range(res_y)])
            comp.wait()

            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_incoherent_infT_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
                   + "_res_x" + str(res_x) + "_res_y" + str(res_y) + ".npz"
            np.savez_compressed(name, metric=metric)
            ...
            pool = mp.Pool(processes=number_of_processes)
            comp = pool.map_async(fill_metric_chi_metric, [(i, j) for i in range(res_x) for j in range(res_y)])
            comp.wait()

            metric = np.array(metric_grid).reshape((res_x, res_y, 2, 2))
            name = "metrics_chi_metric_ltfi_z1z_L" + str(L) + "_l" + str(l) + "_order" + str(order) \
                   + "_res_x" + str(res_x) + "_res_y" + str(res_y) + ".npz"
            np.savez_compressed(name, metric=metric)
