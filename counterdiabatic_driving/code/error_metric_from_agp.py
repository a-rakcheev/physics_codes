...
r = 5                               # evaluate up to range (needs to be <= l)
...
with zipfile.ZipFile(name_zip_op) as zipper_op:
    ...
    mat_name = op_name_1 + "_TP_L=" + str(L) + "_k=" + k_name + "_p=" + str(par) + ".npz"
    with io.BufferedReader(zipper_op.open(mat_name, mode='r')) as f_op:
        data = np.load(f_op)
        indptr = data["indptr"]
        indices = data["idx"]
    ...
    h_deriv_1 = sign_1 * factor_1 * sp.csr_matrix((val, indices, indptr), shape=(size, size))

    mat_name = op_name_2 + "_TP_L=" + str(L) + "_k=" + k_name + "_p=" + str(par) + ".npz"
    with io.BufferedReader(zipper_op.open(mat_name, mode='r')) as f_op:
        data = np.load(f_op)
        indptr = data["indptr"]
        indices = data["idx"]
    ...
    h_deriv_2 = sign_2 * factor_2 * sp.csr_matrix((val, indices, indptr), shape=(size, size))
...
name = prefix + "ltfi_full_ed_k=0_p=1_L=" + str(L) + "_h_res=" + str(res_1) + "_g_res=" + str(res_2) + ".npz"
data = np.load(name)
evec = data["evec"]
ev = data["ev"]
...
metric = np.zeros((res_1 // step1, res_2 // step2, 2, 2), dtype=np.complex128)
...
for i in range(res_1 // step1):
    ...
    for j in range(res_2 // step2):
        ...
        ground_state = evec[i * step1, j * step2, :, 0]

        # sum over excited states
        for n in range(1, size):

            state_n = evec[i * step1, j * step2, :, n]
            omega_n0 = ev[i * step1, j * step2, n] - ev[i * step1, j * step2, 0]

            # create operators

            if r == 0:
                operator_1 = 1.j * h_deriv_1 / omega_n0
                operator_2 = -1.j * h_deriv_2 / omega_n0
                ...
                operator_1 = 1.j * h_deriv_1 / omega_n0 - A_1
                A_1 = None
                ...
                operator_2 = -1.j * h_deriv_2 / omega_n0 - A_2
                A_2 = None

            metric[i, j, 0, 0] += np.absolute(ground_state.T.conj() @ operator_1 @ state_n) ** 2
            metric[i, j, 1, 1] += np.absolute(state_n.T.conj() @ operator_2 @ ground_state) ** 2
            metric[i, j, 0, 1] += (ground_state.T.conj() @ operator_1 @ state_n) * \
                                  (state_n.T.conj() @ operator_2 @ ground_state)
...
plt.savefig("ltfi_error_metric_agp_L=" + str(L) + "_l=" + str(l) + "_r=" + str(r) + ".pdf", format="pdf")
# plt.show()